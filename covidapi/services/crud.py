...
class JHCRUD:
    def get_daily_report(self, db: Session, daily_report_id: int):
        return (
            db.query(models.JHDailyReport)
            .filter(models.JHDailyReport.id == daily_report_id)
            .first()
        )
    ...
    def get_daily_reports(
        ...
        query = db.query(models.JHDailyReport).join(models.JHRegionInfo)
        ...
        if country_code_iso2:
            query = query.filter(
                models.JHRegionInfo.country_code_iso2 == country_code_iso2
            )
        if country_code_iso3:
            query = query.filter(
                models.JHRegionInfo.country_code_iso2 == country_code_iso3
            )
        if scope:
    ...
    def get_daily_report_by_region_and_date(
        ...
        if jh_id:
            dr = db.query(models.JHDailyReport).filter(
                models.JHDailyReport.jh_id == jh_id,
