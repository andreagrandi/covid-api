...
def remove_sar(region_names):
    ...
    return RegionNames(
        ...
        province_state=region_names.province_state.replace(" SAR", "")
        if region_names.province_state
        else None,
...
def fix_fake_provinces(region_names):
    ...
    if region_names.province_state in PROVINCE_DUPLICATES_COUNTRY:
        return RegionNames(
            ...
            if region_names.province_state == "Taiwan"
            else region_names.country_region,
            province_state=None,
            admin2=None,
        )
...
def correct_typos(region_names):
    ...
    """
    country_region = COUNTRY_MAP.get(
        region_names.country_region, region_names.country_region
    )
