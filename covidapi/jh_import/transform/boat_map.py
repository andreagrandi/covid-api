...
def map_boat_passengers(region_match):
    ...
    grand_princess_to_country = ("Grand Princess Cruise Ship",)
    ...
    if (
        region_match.country_region in ("US", "Canada")
        and region_match.province_state in diamond_princess_to_country
    ):
        return RegionNames(
        ...
        region_match.country_region in ("US", "Canada")
        and region_match.province_state in grand_princess_to_country
    ):
        return RegionNames(
            country_region=region_match.country_region,
    ...
    elif (region_match.province_state in diamond_princess) and (
        region_match.country_region in united_federation_of_cruise_ships
    ):
        # The Diamond Princess got promoted from a province into a country
        return RegionNames(
            country_region="Diamond Princess", province_state=None, admin2=None
        )
    elif (
